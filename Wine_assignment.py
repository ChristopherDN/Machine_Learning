...
dataFrame = pd.read_csv('/Users/christophernielsen/PycharmProjects/Machine_Learning/winequalityN.csv') # replace with your file
...
X = dataFrame.iloc[:,:-1] # select relevant rows and columns to X (here for example all rows and columns 5,6,7,8,9,10 and 11)
...
model = Sequential()
model.add(Dense(50,activation='relu')) # 4 outputs. It will automatically adapt to number inputs
model.add(Dense(50,activation='relu'))
model.add(Dense(units=1, activation='linear')) # Final output node for prediction. In this case, only one output neuron
...
#
model.fit(X_train,y_train,epochs=200, batch_size=30,  verbose=0)  # does the actual WORK !. verbose=1 will show output. 0 = no output.
...
new_value = [[8.6, 0.23, 0.4, 4.2, 0.035, 17, 109, 0.9947, 3.14, 0.53, 9.7,0,1]] # enter new data in 2D array. Only numbers + dummy variables.
