...
env = gym.make('MountainCar-v0')  # load this particular game. Other games are available. (render_mode="rgb_array")
env.reset()
...
def intial_population():
    ...
    for _ in range(initial_games):
        ...
        for _ in range(goal_steps):
            ...
            action = random.randrange(0,
                                      2)  # will start with random number from 0 to 1. 0 = push car left. 1 = push car right.
    ...
    np.save('savedcart.npy', np.array(training_data))  # saves training_data to file.
    print('-----------------------------------------------')
...
trainingData = np.load('savedcart.npy', allow_pickle=True)
